...
const Signup = () => {
    ...
    const submitHandler = async (e) => {
        ...
        try {
            ...
            const res = await axios.post("https://thr-backend.onrender.com/api/v1/user/register", formData, {
  ...
  <Label style={{ color: '#7f99b5', marginLeft: '18px' }}>Profile</Label>
  <Input
    accept="image/*"
    type="file"
    onChange={changeFileHandler}
    className="cursor-pointer"
    aria-label="Choose a file" // Accessible label
  />
</div>
  ...
  
  {
    loading ? (
      ...
      <Button type="submit" className="w-full my-4 border-[#edb526] text-[#edb526] hover:bg-[#edb526] hover:text-white">Signup</Button>
    )
  }
